...
pub use xcm::{
	prelude::{
		Asset, InteriorLocation, Location, OriginKind, Outcome, VersionedLocation, VersionedXcm,
		XcmError, XcmVersion,
	},
	DoubleEncoded,
};
		...
		MessageDispatch<DispatchLevelResult = XcmBlobMessageDispatchResult>,
{
	fn get_source_outbound_messages() -> Vec<BridgeMessage> {
		...
		for lane in active_outbound_lanes {
			let latest_generated_nonce = OutboundLanes::<S, SI>::get(lane)
				.unwrap()
				.latest_generated_nonce;
			let latest_received_nonce = OutboundLanes::<S, SI>::get(lane)
				.unwrap()
			...
			(latest_received_nonce + 1..=latest_generated_nonce).for_each(|nonce| {
				...
				let message = BridgeMessage {
					lane_id: LaneIdWrapper(lane).into(),
					nonce,
					payload,
				};
...
pub use bifrost_asset_registry;

#[macro_export]
macro_rules! impl_currencies_helpers_for_parachain {
	($chain:ident) => {
		$crate::impls::paste::paste! {
			impl<N: $crate::impls::Network> $chain<N> {
				/// Create assets using sudo `AssetRegistry::force_set_location()`
				pub fn force_set_location(
					currency_id: bifrost_primitives::CurrencyId,
					location: $crate::impls::VersionedLocation,
					weight: $crate::impls::Weight,
				) {
					use $crate::impls::Inspect;
					let sudo_origin = <$chain<N> as $crate::impls::Chain>::RuntimeOrigin::root();
					<Self as $crate::impls::TestExt>::execute_with(|| {
						$crate::impls::assert_ok!(
							<Self as [<$chain ParaPallet>]>::AssetRegistry::force_set_location(
								sudo_origin,
								currency_id.clone(),
						...
						$crate::impls::assert_expected_events!(
							Self,
							vec![
								RuntimeEvent::<N>::AssetRegistry(
									$crate::impls::bifrost_asset_registry::Event::LocationSet {
										currency_id,
										..
									}
								) => { currency_id: *currency_id == currency_id.clone(), },
							]
						);
					});
				}
				...
				pub fn mint_asset(
					...
					<Self as $crate::impls::TestExt>::execute_with(|| {
						$crate::impls::assert_ok!(<Self as [<$chain ParaPallet>]>::Currencies::deposit(
							currency_id,
							&beneficiary,
							amount_to_mint
						));
