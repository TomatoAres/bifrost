...
export BIFROST_PARACHAIN_BINARY=$LOCAL_BRIDGE_TESTING_PATH/bin/bifrost
...
#export TEST_DIR=`~/tmp/bridges-tests-run-95gIg`
echo -e "Test folder: $TEST_DIR\n"

echo "Zombienet ==================================="

echo ${BASH_SOURCE%/*}/tests/$test/run.sh
sh ${BASH_SOURCE%/*}/tests/$test/run.sh