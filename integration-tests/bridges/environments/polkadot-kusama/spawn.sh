...
if [[ $init -eq 1 ]]; then
  ...
  kusama_init_pid=$!
  wait $polkadot_init_pid $kusama_init_pid
  ...
  wait $polkadot_init_pid $kusama_init_pid
#
#  $helper_script init-bifrost-polkadot-local >> $polkadot_init_log 2>&1 &
#  polkadot_init_pid=$!
#  $helper_script init-bifrost-kusama-local >> $kusama_init_log 2>&1 &
#  kusama_init_pid=$!
#  wait $polkadot_init_pid $kusama_init_pid
...
wait $polkadot_pid $kusama_pid $finality_relayer_pid $parachains_relayer_pid $messages_relayer_pid
kill -9 -$$