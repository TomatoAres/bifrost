...
fn to_vtoken_should_work() {
	...
	assert_eq!(
		native_asg.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vtoken_ksm.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		token_ksm.to_vtoken(),
	...
	assert_eq!(
		stable_kusd.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vstoken_eth.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vsbond_ksm.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
	...
	assert_eq!(
		vtoken2_dot.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vstoken2_dot.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vsbond2_dot.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		foreign_asset.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(blp.to_vtoken(), Err(CurrencyConversionError::InvalidToken));
	assert_eq!(
		stable_lp_token.to_vtoken(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		lp_token.to_vtoken(),
...
fn to_token_should_work() {
	...
	assert_eq!(
		stable_kusd.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vstoken_eth.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vsbond_ksm.to_token(),
	...
	assert_eq!(
		vstoken2_dot.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		vsbond2_dot.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		foreign_asset.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(blp.to_token(), Err(CurrencyConversionError::InvalidToken));
	assert_eq!(
		stable_lp_token.to_token(),
		Err(CurrencyConversionError::InvalidToken)
	);
	assert_eq!(
		lp_token.to_token(),
