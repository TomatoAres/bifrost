...
pub struct SlpMigrationV6<T>(PhantomData<T>);
impl<T: Config> OnRuntimeUpgrade for SlpMigrationV6<T> {
	fn on_runtime_upgrade() -> Weight {
		...
		if on_chain_version == 5 && in_code_version == 6 {
...
pub struct UpgradeStorageVersion<T>(PhantomData<T>);
