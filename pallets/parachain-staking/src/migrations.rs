...
use sp_runtime::{
	...
	Perbill, Percent, SaturatedConversion, TryRuntimeError,
};
...
impl<T: Config> OnRuntimeUpgrade for RemoveDelegatorReserveToLockAndCollatorReserveToLock<T> {
	...
	fn post_upgrade(_state: Vec<u8>) -> Result<(), TryRuntimeError> {
		assert_eq!(
			DelegatorReserveToLockMigrations::<T>::iter_keys().count(),
			0
		);
...
/// Migration to exit all Candidates
pub struct LeaveAllCandidates<T>(PhantomData<T>);
impl<T: Config + pallet_balances::Config> OnRuntimeUpgrade for LeaveAllCandidates<T> {
	fn on_runtime_upgrade() -> Weight {
		log::info!(target: "LeaveAllCandidates", "running migration to exit all Candidates");
		...
		// get total counted for all candidates
		for (account, state) in <CandidateInfo<T>>::iter() {
			match Pallet::<T>::do_execute_leave_candidates(state.clone(), account.clone()) {
				Ok(_) => {
					total_candidates += 1;
				}
				Err(e) => {
			...
			weight = weight.saturating_add(
				T::DbWeight::get().reads_writes(delegation_count.into(), delegation_count.into()),
			);
		}

		// Set the length to u32::MAX to skip executing hooks
		let mut round = Round::<T>::get();
		round.length = u32::MAX;
		Round::<T>::put(round);
		weight = weight.saturating_add(T::DbWeight::get().reads_writes(1, 1));

		// Final consistency check: DelegatorState and CandidateInfo must be empty
		let has_delegators = DelegatorState::<T>::iter().next().is_some();
		let has_candidates = CandidateInfo::<T>::iter().next().is_some();
		assert!(
			!has_delegators,
			"Migration error: DelegatorState not fully cleared"
		);
		assert!(
			!has_candidates,
			"Migration error: CandidateInfo not fully cleared"
		);
	...
	fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
		...
		let mut stakes: BTreeMap<T::AccountId, BalanceOf<T>> = BTreeMap::new();

		// Record each candidate and their self-bond
		for (acc, info) in CandidateInfo::<T>::iter() {
			stakes.insert(acc.clone(), info.bond);
		}

		// Record the total amount of bond delegated by each delegator.
		for (delegator, state) in DelegatorState::<T>::iter() {
			let total: BalanceOf<T> = state
				.delegations
				.0
				.iter()
				.fold(Zero::zero(), |acc, b| acc + b.amount);
			stakes
	...
	fn post_upgrade(data: Vec<u8>) -> Result<(), TryRuntimeError> {
		...
		let stakes_before: BTreeMap<T::AccountId, BalanceOf<T>> = Decode::decode(
			&mut data.as_slice(),
		)
		.expect("the state parameter should be something that was generated by pre_upgrade");

		for (acc, amount_before) in stakes_before.iter() {
			let balance = T::Currency::free_balance(acc);
			let total = T::Currency::total_balance(acc);

			ensure!(
				balance >= *amount_before,
				"❌ post_upgrade failed: balance dropped"
			);

			let locks = Locks::<T>::get(acc);
			// Check if there is any remaining DELEGATOR_LOCK_ID or COLLATOR_LOCK_ID.
			let has_collator_lock = locks.iter().any(|l| l.id == COLLATOR_LOCK_ID);
			let has_delegator_lock = locks.iter().any(|l| l.id == DELEGATOR_LOCK_ID);
			...
			locks);
				...
				!has_collator_lock,
				"❌ post_upgrade: Collator lock still present for account"
			);
			ensure!(
				!has_delegator_lock,
				"❌ post_upgrade: Delegator lock still present for account"
			);
		}

		// Check if CandidateInfo has been cleared
		let has_candidate_info = CandidateInfo::<T>::iter().next().is_some();
		ensure!(
			!has_candidate_info,
			"❌ post_upgrade check failed: CandidateInfo not empty"
		);

		// Check if DelegatorState has been cleared
		let has_delegator_state = DelegatorState::<T>::iter().next().is_some();
		ensure!(
			!has_delegator_state,
			"❌ post_upgrade check failed: DelegatorState not empty"
		);

		// Check if TopDelegations has been cleared
		let has_top_delegations = TopDelegations::<T>::iter().next().is_some();
		ensure!(
			!has_top_delegations,
			"❌ post_upgrade check failed: TopDelegations not empty"
		);

		// Check if BottomDelegations has been cleared
		let has_bottom_delegations = BottomDelegations::<T>::iter().next().is_some();
		ensure!(
			!has_bottom_delegations,
			"❌ post_upgrade check failed: BottomDelegations not empty"
		);

		// Check if DelegationScheduledRequests has been cleared
		let has_scheduled = DelegationScheduledRequests::<T>::iter().next().is_some();
		ensure!(
			!has_scheduled,
			"❌ post_upgrade check failed: DelegationScheduledRequests not empty"
		);

		log::info!(
			target: "parachain-staking-migration",
			"✅ post_upgrade check passed: all staking-related storage entries are cleared."
		);
