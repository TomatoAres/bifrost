...
fn set_parachain_bond_reserve_percent_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			ParachainBondInfo::<Test>::get().percent,
...
fn execute_leave_delegators_removes_delegations_from_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DelegatorState::<Test>::get(1).unwrap().delegations.0.len(),
				4usize
			);
			assert_ok!(ParachainStaking::schedule_leave_delegators(
				RuntimeOrigin::signed(1)
...
fn delegator_bond_more_updates_candidate_state_top_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				TopDelegations::<Test>::get(1).unwrap().delegations[0].owner,
				2
			);
			assert_eq!(
				TopDelegations::<Test>::get(1).unwrap().delegations[0].amount,
				10
			);
...
fn execute_revoke_delegation_removes_delegation_from_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(CandidateInfo::<Test>::get(1)
				.expect("exists")
				.delegation_count
				.is_zero());
		});
...
fn multiple_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DelegatorState::<Test>::get(7).unwrap().delegations.0.len(),
				2usize
			);
			assert_eq!(DelegatorState::<Test>::get(6).unwrap().total(), 40);
			assert_eq!(
				DelegatorState::<Test>::get(6).unwrap().delegations.0.len(),
				4usize
			);
...
fn execute_leave_candidate_removes_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(!DelegationScheduledRequests::<Test>::get(&2)
				.iter()
				.any(|x| x.delegator == 3));
